...
#define CTLCFG_DENALI_PI_(x)				((x << 2U) + DDRSS_PI_REGISTER_BLOCK_OFFS)
...
#define CTLCFG_DENALI_PHY_(x)				((x << 2U) + DDRSS_DATA_SLICE_0_REGISTER_BLOCK_OFFS)
...
__wkupsramfunc void poll_for_init_completion(struct emif_handle_s *h)
{
	...
	while (((mmio_read_32(h->ctl_cfg_base_addr + CTLCFG_DENALI_CTL_(342))) & 0x02000000) != 0x02000000) {
	}
}
...
__wkupsramfunc void put_ddr_in_sr(bool enable)
{
	if (enable) {
		...
		write_mmr_field(DDRSS0_CTRL_BASE + CTLCFG_DENALI_CTL_(158), LP_MODE_LONG_SELF_REFRESH_PHY_CTRL, 7U, 8U);
		...
		/* Program Self Refresh mode */
		write_mmr_field(DDRSS0_CTRL_BASE + CTLCFG_DENALI_CTL_(158), LP_MODE_LONG_SELF_REFRESH_EXIT, 7U, 8U);
		/* Poll for Self Refresh Mode change */
		write_mmr_field(DDRSS0_CTRL_BASE + CTLCFG_DENALI_CTL_(353), 0x0U, 16U, 16U);
		lp_status = (mmio_read_32(DDRSS0_CTRL_BASE + CTLCFG_DENALI_CTL_(337)) & 0x10000U);
		while (lp_status != 0x10000U) {
...
__wkupsramfunc void save_ddr_registers(struct emif_handle_s *h)
{
	...
	uint32_t DDR_PI_REG_BASE = (h->ctl_cfg_base_addr) + DDRSS_PI_REGISTER_BLOCK_OFFS;
	uint32_t DDR_PHY_DATA_SLICE_0_REG_BASE = (h->ctl_cfg_base_addr) + DDRSS_DATA_SLICE_0_REGISTER_BLOCK_OFFS;
	uint32_t DDR_PHY_DATA_SLICE_1_REG_BASE = (h->ctl_cfg_base_addr) + DDRSS_DATA_SLICE_1_REGISTER_BLOCK_OFFS;
	uint32_t DDR_PHY_ADDR_SLICE_0_REG_BASE = (h->ctl_cfg_base_addr) + DDRSS_ADDRESS_SLICE_0_REGISTER_BLOCK_OFFS;
	uint32_t DDR_PHY_ADDR_SLICE_1_REG_BASE = (h->ctl_cfg_base_addr) + DDRSS_ADDRESS_SLICE_1_REGISTER_BLOCK_OFFS;
...
__wkupsramfunc void enter_lpm_self_refresh(struct emif_handle_s *h)
{
	...
	/* Program Self Refresh mode */
	mmio_write_32(h->ctl_cfg_base_addr + CTLCFG_DENALI_CTL_(158), (LP_MODE_LONG_SELF_REFRESH << 8));

	/* Poll for Self Refresh Mode change */
	while (lp_status != 0x4E) {
		lp_status = ((mmio_read_32(h->ctl_cfg_base_addr + CTLCFG_DENALI_CTL_(167)) & 0x7F00) >> 8);
	}
}
