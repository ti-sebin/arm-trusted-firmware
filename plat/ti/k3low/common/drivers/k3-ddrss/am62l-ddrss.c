...
#define CLKCHNG_REQ_MASK GENMASK(7, 7)
#define CLKCHNG_REQ_ACTIVE 0x01U
#define CLKCHNG_REQ_INACTIVE 0x00U
#define CLKCHNG_REQTYPE_MASK GENMASK(1, 0)
#define CLKCHNG_REQ_FREQ1 0x01U
#define CLKCHNG_REQ_FREQ2 0x02U
#define CLKCHNG_REQ_FREQ0 0x00U
#define CLKCHNG_ACK_DONE 0x01U
#define CLKCHNG_ACK_NONE 0x00U
...
static int set_ddr_pll_div(unsigned int div)
{
	...
	/* Set reset to high */
	reg_val = mmio_read_32((uintptr_t)(PLL_CFG + PLL0_HSDIV2_CTRL));
	set_val = (reg_val & ~PLL_HSDIV_RESET_MASK) | FIELD_PREP(PLL_HSDIV_RESET_MASK, 1);
	mmio_write_32((uintptr_t)(PLL_CFG + PLL0_HSDIV2_CTRL), set_val);
...
static void k3_lpddr4_freq_update(struct k3_ddrss_desc *ddr)
{
	...
	for (counter = 0; counter < ddr->ddr_fhs_cnt; counter++) {
		...
		req = FIELD_GET(CLKCHNG_REQ_MASK, reg_val);
		while (req == CLKCHNG_REQ_INACTIVE) {
			reg_val = mmio_read_32((uintptr_t)(DDRSS_CTRL_MMR + DDR4_FSP_CLKCHNG_REQ));
			req = FIELD_GET(CLKCHNG_REQ_MASK, reg_val);
		}

		reg_val = mmio_read_32((uintptr_t)(DDRSS_CTRL_MMR + DDR4_FSP_CLKCHNG_REQ));
		req_type = FIELD_GET(CLKCHNG_REQTYPE_MASK, reg_val);
		if (req_type == CLKCHNG_REQ_FREQ1)
			ddrss_set_pll(ddr->ddr_freq1);
		else if (req_type == CLKCHNG_REQ_FREQ2)
			ddrss_set_pll(ddr->ddr_freq2);
		else if (req_type == CLKCHNG_REQ_FREQ0)
			ddrss_set_pll(ddr->ddr_freq0);
		else
			WARN("invalid DDR freq request type\n");

		mmio_write_32((uintptr_t)(DDRSS_CTRL_MMR + DDR4_FSP_CLKCHNG_ACK),  CLKCHNG_ACK_DONE);
		reg_val = mmio_read_32((uintptr_t)(DDRSS_CTRL_MMR + DDR4_FSP_CLKCHNG_REQ));
		req = FIELD_GET(CLKCHNG_REQ_MASK, reg_val);
		while (req == CLKCHNG_REQ_ACTIVE) {
			reg_val = mmio_read_32((uintptr_t)(DDRSS_CTRL_MMR + DDR4_FSP_CLKCHNG_REQ));
			req = FIELD_GET(CLKCHNG_REQ_MASK, reg_val);
		}
		mmio_write_32((uintptr_t)(DDRSS_CTRL_MMR + DDR4_FSP_CLKCHNG_ACK), CLKCHNG_ACK_NONE);
	}
